...
<head>
    ...
    <%
        request.setAttribute("request", "test request");
    %>
</head>
<body>
...
<h1>时间：<%= new java.util.Date() %>
</h1>

<%
    session.setAttribute("name", "session对象");//使用session对象,设置session对象的属性
    out.print(pageContext.getSession().getAttribute("name") + "<br/>");//获取session对象的属性
...
/*
    pageContext.getRequest().getRequestDispatcher("/jsp/welcome.jsp").forward(request, response);
*/
    pageContext.setAttribute("name", "cc", PageContext.PAGE_SCOPE);


    out.println("name:" + pageContext.getAttribute("name"));
%>
<%--<%
